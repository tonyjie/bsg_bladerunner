...
int kernel_float_vec_mul(int path_cnt, int *path_idx, int **paths, int nodes, double* X, double* Y, int eta_cnt, double* etas, double* zetas) 
{
	// a seed source for the random number engine
    std::mt19937 gen(23462346 + __bsg_id); // mersenne_twister_engine seeded with rd()
	int space = 0;
	for(int i = 0; i < path_cnt ; i++) space = std::max(space, path_idx[i]);
    int space_max = 1000;
    int space_quantization_step = 100;
    double theta = 0.99;
    for(int eta_idx = 0; eta_idx < 2 ; eta_idx++)
    {   
        auto eta = etas[eta_idx];
        for(int iter = 0; iter < 1; iter++)
        {    
            auto path = gen()%path_cnt;
            int node_a_idx = gen()%path_idx[path];
            //Cooling and Zipfian Stuff
            int node_b_idx = node_a_idx;
            if (eta_idx >= 0.9 * eta_cnt || gen()%2) 
            {
            ...
            auto pos_in_path_a = paths[path][node_a_idx*2+1];
            auto pos_in_path_b = paths[path][node_b_idx*2+1];
